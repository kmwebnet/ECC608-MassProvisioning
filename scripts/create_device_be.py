...
def create_device(device_file, device_key_file, signer_file, signer_key_file, root_file, root_key_file, ou_file ,cn ):
    ...
    with open(ou_file, 'rb') as f:
        o = f.readline().decode("utf-8")
        o = "".join(o).replace('\r', '').replace('\n', '')

    with open(cn, 'rb') as f:
        cn = f.readline().decode("utf-8")
        cn = "".join(cn).replace('\r', '').replace('\n', '')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ou', default='ou.txt', help='organization unit')
    args = parser.parse_args()

    create_device(args.device, args.devicekey, args.signer, args.signerkey, args.root, args.rootkey, ou_file)
